...
struct TradeLogEntry
{
   ...
   double entryEntropy;
   double entryPhysicsScore;    // NEW: Evidence-based weighted physics score at entry
   string entryZone;
   string entryRegime;
   double entrySpread;
...
private:
   ...
   bool WriteTradeHeader()
   {
      ...
      // Write header (56 columns - Enhanced with PhysicsScore)
      FileWrite(handle,
         "EAName", "EAVersion",
...
public:
   ...
   bool LogSignal(SignalLogEntry &entry)
   {
      ...
      FileWrite(handle,
         ...
         entry.entropy, entry.jerk, entry.physicsScore,
         entry.speedSlope, entry.accelerationSlope, entry.momentumSlope, entry.confluenceSlope, entry.jerkSlope,
   ...
   bool LogTrade(TradeLogEntry &entry)
   {
      ...
      FileWrite(handle,
         ...
         entry.entryQuality, entry.entryConfluence, entry.entryMomentum, entry.entryEntropy, entry.entryPhysicsScore,
