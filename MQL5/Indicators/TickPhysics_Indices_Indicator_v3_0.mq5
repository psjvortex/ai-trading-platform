...
//| Indices-Optimized Elite Momentum Physics Engine                  |
//| Version 3.0 - Optimized for NAS100, US30, SPX500, DAX, FTSE     |
//| Settings tuned for stock index volatility characteristics       |
//+------------------------------------------------------------------+
...
//============================= INPUTS ==============================//
input group "=== INDICES OPTIMIZED SETTINGS ==="
input bool   InpIndicesMode = true;  // Enable Indices Optimizations (NAS100/US30/SPX500)
//input string InpAsset = "NAS100";   // Asset (NAS100/US30/SPX500/DAX)

input group "=== Physics Core ==="
input int InpSpeedPeriod = 4;       // Speed Period (indices: 4 vs forex: 3 vs crypto: 5)
input int InpSpeedSmooth = 7;       // Speed Smoothing (indices: 7 vs forex: 5 vs crypto: 8)
input double InpSpeedMultiplier = 130.0;  // Speed Multiplier (indices: 130 vs forex: 100 vs crypto: 150)
input double InpAccelMultiplier = 200.0;  // Accel Multiplier (indices: 200 vs forex: 150 vs crypto: 250)
...
int OnInit()
{
   ...
   string modeName = InpIndicesMode ? " [INDICES]" : " [STANDARD]";
   IndicatorSetString(INDICATOR_SHORTNAME, "TickPhysics Indices v2.1" + modeName);
   ...
   Print("✅ TickPhysics Indices v2.1 initialized successfully!");
   Print("📊 Asset: ", _Symbol);
   Print("⚡ Indices mode: ", InpIndicesMode ? "ENABLED" : "DISABLED");
                ...
                const long &volume[], const int &spread[])
{
   ...
   // Apply indices volatility multiplier
   if(InpIndicesMode) {
...
void CalculateConfluence(int i, int total, double highThr)
{
   if(InpShowConfluence && i <= total - InpSpeedPeriod - 2 && i < ArraySize(ConfluenceBuffer))
   {
      ...
      // QA FIX: Original check (DistanceROC * Speed > 0) failed for shorts because DistanceROC is always positive.
      // Changed to check for EXPANDING volatility (DistanceROC increasing)
      if(i < ArraySize(DistanceROCBuffer) && i + 1 < ArraySize(DistanceROCBuffer) && DistanceROCBuffer[i] > DistanceROCBuffer[i+1]) confluence++;
...
void OnDeinit(const int reason)
{
   ...
   Print("TickPhysics Indices v2.1 deinitialized. Reason: ", reason);
}
//+------------------------------------------------------------------+
