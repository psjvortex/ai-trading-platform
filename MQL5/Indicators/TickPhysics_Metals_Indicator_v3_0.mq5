...
//| Metals-Optimized Elite Momentum Physics Engine                   |
//| Version 3.0 - Optimized for Gold, Silver, Platinum, Palladium   |
//| Settings tuned for precious metals volatility characteristics   |
//+------------------------------------------------------------------+
...
//============================= INPUTS ==============================//
input group "=== METALS OPTIMIZED SETTINGS ==="
input bool   InpMetalsMode = true;  // Enable Metals Optimizations (Gold/Silver/Platinum)
//input string InpAsset = "XAUUSD";   // Asset (XAUUSD/XAGUSD/XPTUSD)

input group "=== Physics Core ==="
input int InpSpeedPeriod = 6;       // Speed Period (metals: 6 vs crypto: 5 vs forex: 3)
input int InpSpeedSmooth = 6;       // Speed Smoothing (metals: 6 vs crypto: 8 vs forex: 5)
input double InpSpeedMultiplier = 120.0;  // Speed Multiplier (metals: 120 vs crypto: 150)
input double InpAccelMultiplier = 180.0;  // Accel Multiplier (metals: 180 vs crypto: 250)

input group "=== ATR Adaptive Scaling ==="
input int InpATRPeriod = 18;        // ATR Period (metals: 18 vs crypto: 20 vs forex: 14)
input bool InpAutoRefPoints = true;
input double InpATRRefPoints = 800.0;  // Metals default (Gold: ~$8/pip)
input double InpATRRef_Crypto = 2000.0; // BTC ~2000 points
input double InpATRRef_Forex = 100.0;
input double InpATRRef_Index = 250.0;
input double InpATRRef_Metals = 800.0; // XAUUSD ~800 points (~$8 move)

input group "=== Momentum & Quality ==="
input double InpSpeedWeight = 0.50;     // Speed Weight (metals: 0.50 vs crypto: 0.55)
input double InpAccelWeight = 0.50;     // Accel Weight (metals: 0.50 vs crypto: 0.45)
input int InpMomentumSmooth = 6;        // Momentum Smoothing (metals: 6 vs crypto: 5)
input double InpMomentumScale = 2.0;    // Momentum Scale (metals: 2.0 vs crypto: 2.5)
input int InpTrendQualityPeriod = 18;   // Trend Quality Period (metals: 18 vs crypto: 15)
input double InpQualityMagWeight = 0.30; // Quality Magnitude Weight (metals: 0.30 vs crypto: 0.25)

input group "=== Distance & Jerk ==="
input int InpDistanceROCPeriod = 8;     // Distance ROC Period (metals: 8 vs crypto: 7)
input double InpDistanceROCScale = 65.0; // Distance ROC Scale (metals: 65 vs crypto: 75)
input int InpJerkSmooth = 7;            // Jerk Smoothing (metals: 7 vs crypto: 6)

input group "=== Thresholds ==="
input bool InpUseAdaptiveThresh = true;
input double InpThresholdATRMult = 1.0;  // Threshold ATR Mult (metals: 1.0 vs crypto: 1.2)
input double InpHighThreshold = 65.0;    // High Threshold (metals: 65 vs crypto: 75)
input double InpLowThreshold = -65.0;    // Low Threshold (metals: -65 vs crypto: -75)
input double InpVolHighMult = 1.6;       // Vol High Multiplier (metals: 1.6 vs crypto: 1.8)
...
input double InpVolLowThreshold = 0.7;   // Vol Low Threshold (metals: 0.7 vs crypto: 0.6)
input double InpVolHighThreshold = 1.4;  // Vol High Threshold (metals: 1.4 vs crypto: 1.5)

input group "=== Divergence Detection ==="
input bool InpShowDivergence = true;
input int InpDivergenceLookback = 30;    // Divergence Lookback (metals: 30 vs crypto: 25)
...
string HUD_NAME = "TPX_Metals_HUD_v2";
string HUD_BG_NAME = "TPX_Metals_HUD_BG_v2";
...
void TriggerAlert(string message)
{
   ...
   Alert("� TickPhysics Metals: ", message);
...
int OnInit()
{
   ...
   string modeName = InpMetalsMode ? " [METALS]" : " [STANDARD]";
   IndicatorSetString(INDICATOR_SHORTNAME, "TickPhysics Metals v2.1" + modeName);
   ...
   Print("✅ TickPhysics Metals v2.1 initialized successfully!");
   Print("📊 Asset: ", _Symbol);
   Print("⚡ Metals mode: ", InpMetalsMode ? "ENABLED" : "DISABLED");
                ...
                const long &volume[], const int &spread[])
{
   ...
   // Apply metals volatility multiplier
   if(InpMetalsMode) {
...
void OnDeinit(const int reason)
{
   ...
   Print("TickPhysics Metals v2.1 deinitialized. Reason: ", reason);
}
//+------------------------------------------------------------------+
