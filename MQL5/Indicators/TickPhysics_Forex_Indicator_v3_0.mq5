...
//| Forex-Optimized Elite Momentum Physics Engine                    |
//| Version 3.0 - Optimized for Major/Minor Currency Pairs          |
//| Settings tuned for forex market volatility characteristics      |
//+------------------------------------------------------------------+
...
//============================= INPUTS ==============================//
input group "=== FOREX OPTIMIZED SETTINGS ==="
input bool   InpForexMode = true;  // Enable Forex Optimizations (EUR/USD/GBP/JPY pairs)
//input string InpAsset = "EURUSD";   // Asset (EURUSD/GBPUSD/USDJPY)

input group "=== Physics Core ==="
input int InpSpeedPeriod = 3;       // Speed Period (forex: 3 vs metals: 6 vs crypto: 5)
input int InpSpeedSmooth = 5;       // Speed Smoothing (forex: 5 vs metals: 6 vs crypto: 8)
input double InpSpeedMultiplier = 100.0;  // Speed Multiplier (forex: 100 vs crypto: 150)
input double InpAccelMultiplier = 150.0;  // Accel Multiplier (forex: 150 vs crypto: 250)
...
int OnInit()
{
   ...
   string modeName = InpForexMode ? " [FOREX]" : " [STANDARD]";
   IndicatorSetString(INDICATOR_SHORTNAME, "TickPhysics Forex v2.1" + modeName);
   ...
   Print("✅ TickPhysics Forex v2.1 initialized successfully!");
   Print("📊 Asset: ", _Symbol);
   Print("⚡ Forex mode: ", InpForexMode ? "ENABLED" : "DISABLED");
                ...
                const long &volume[], const int &spread[])
{
   ...
   // Apply forex volatility multiplier
   if(InpForexMode) {
...
void OnDeinit(const int reason)
{
   ...
   Print("TickPhysics Forex v2.1 deinitialized. Reason: ", reason);
}
//+------------------------------------------------------------------+
