//+------------------------------------------------------------------+
//|                       TP_Integrated_EA_Crossover_4_1_6.mq5       |
//|                      TickPhysics Institutional Framework (ITPF)  |
//|             v4.15 - SLOPE ANALYSIS (QA APPROVED)                 |
//+------------------------------------------------------------------+
#property copyright "Copyright 2025, QuanAlpha"
#property link      "https://github.com/quanalpha/tickphysics"
#property version   "4.16"
#property description "v4.16 EA with Slope Analysis + Universal Elite v3.1 Indicator Support - QA APPROVED"

// EA Version Info (for CSV tracking)
#define EA_NAME "TP_Integrated_EA"
#define EA_VERSION "4.16_SLOPE"

input int MagicNumber = 400416;                       // EA magic number
input string TradeComment = "TP_Integrated 4_16";      // Trade comment
...
input int         StopLossPips            = 10000;        // Stop loss (pips) - REALISTIC DEFAULT
...
input double               MinQuality                 = 85.0;            // Min physics quality (v4.0 threshold)
...
input bool                 UseAccelerationFilter      = false;            // Enable acceleration threshold
...
input bool                 UseEvidenceBasedWeights    = false;            // Use evidence-based weights from correlation analysis
input bool                 UseTimeframeSpecificWeights = false;          // Different weights for 1H vs 5M
...
input bool                 UsePhysicsScoreFilter      = false;            // Enable physics score threshold
input double               MinPhysicsScore            = 55.0;            // Min physics score (Q3 threshold: 55-85 optimal)
input bool                 RequireFullConfluence      = false;            // Require 100% confluence (+11.5% win boost)
...
double CalculateSlope(double value0, double value1, double value2, int bars)
{
   ...
   // NOTE: Uses bar 0 for real-time monitoring, but only calculates OnNewBar()
...
{
   // Get values for slope calculation (0 = current, 1 = previous, 2 = 2 bars ago)
   // Uses bar 0 but only called in OnNewBar(), so represents bar close values
   ...
   double speed1 = g_physics.GetSpeed(1);
   ...
   double accel2 = g_physics.GetAcceleration(lookback - 1);
   g_lastAccelerationSlope = CalculateSlope(accel0, accel1, accel2, lookback);
   
   // Momentum slope
   double momentum0 = g_physics.GetMomentum(0);
   double momentum1 = g_physics.GetMomentum(1);
   double momentum2 = g_physics.GetMomentum(lookback - 1);
   ...
   // Jerk slope
   double jerk0 = g_physics.GetJerk(0);
   double jerk1 = g_physics.GetJerk(1);
   double jerk2 = g_physics.GetJerk(lookback - 1);
...
int OnInit()
{
   ...
   if(UseSlopeFilters)
   {
      ...
      if(UseSpeedSlope)
         Print("      → Speed Slope >= ", MinSpeedSlope, " (directional momentum)");
      if(UseAccelerationSlope)
         Print("      → Acceleration Slope >= ", MinAccelerationSlope, " (trend confirmation)");
      if(UseConfluenceSlope)
         Print("      → Confluence Slope >= ", MinConfluenceSlope, " (alignment strengthening)");
      if(UseMomentumSlope)
         Print("      → Momentum Slope >= ", MinMomentumSlope, " (momentum building)");
      if(UseJerkSlope)
         Print("      → Jerk Slope >= ", MinJerkSlope, " (advanced, may be noisy)");
   }
...
int GeneratePhysicsFilterSignal()
{
   ...
   if(UseIsolatedTesting)
   {
      ...
      {
         if(g_lastSpeedSlope > MinSpeedSlope && speed > 0)
         {
            signal = 1;  // BUY
            if(EnableDebugMode)
               Print("🧪 ISOLATED SPEED SLOPE BUY: Slope=", g_lastSpeedSlope, " (threshold=", MinSpeedSlope, ")");
         }
         else if(g_lastSpeedSlope < -MinSpeedSlope && speed < 0)
         {
            signal = -1;  // SELL
            if(EnableDebugMode)
               Print("🧪 ISOLATED SPEED SLOPE SELL: Slope=", g_lastSpeedSlope, " (threshold=-", MinSpeedSlope, ")");
         }
         return signal;
      }
      ...
      {
         if(g_lastAccelerationSlope > MinAccelerationSlope && accel > 0)
         {
            signal = 1;  // BUY
            if(EnableDebugMode)
               Print("🧪 ISOLATED ACCEL SLOPE BUY: Slope=", g_lastAccelerationSlope, " (threshold=", MinAccelerationSlope, ")");
         }
         else if(g_lastAccelerationSlope < -MinAccelerationSlope && accel < 0)
         {
            signal = -1;  // SELL
            if(EnableDebugMode)
               Print("🧪 ISOLATED ACCEL SLOPE SELL: Slope=", g_lastAccelerationSlope, " (threshold=-", MinAccelerationSlope, ")");
         }
         return signal;
      }
      ...
      {
         if(g_lastMomentumSlope > MinMomentumSlope && momentum > 0)
         {
            signal = 1;  // BUY
            if(EnableDebugMode)
               Print("🧪 ISOLATED MOMENTUM SLOPE BUY: Slope=", g_lastMomentumSlope, " (threshold=", MinMomentumSlope, ")");
         }
         else if(g_lastMomentumSlope < -MinMomentumSlope && momentum < 0)
         {
            signal = -1;  // SELL
            if(EnableDebugMode)
               Print("🧪 ISOLATED MOMENTUM SLOPE SELL: Slope=", g_lastMomentumSlope, " (threshold=-", MinMomentumSlope, ")");
         }
         return signal;
      }
      ...
      if(IsolatedSpeedOnly)
      {
         ...
         else if(MathAbs(speed) > MinSpeedSell && speed < 0)
         {
            signal = -1;  // SELL
            if(EnableDebugMode)
      ...
      if(IsolatedAccelOnly)
      {
         ...
         else if(MathAbs(accel) > MinAccelerationSell && accel < 0)
         {
            signal = -1;  // SELL
            if(EnableDebugMode)
      ...
      if(IsolatedMomentumOnly)
      {
         ...
         else if(MathAbs(momentum) > MinMomentumSell && momentum < 0)
         {
            signal = -1;  // SELL
            if(EnableDebugMode)
   ...
   if((UseMAEntry || UsePhysicsEntry) && UsePhysicsFilters && !BypassAllFilters)
   {
      ...
      if(UseSlopeFilters && passFilters)
      {
         ...
         if(UseSpeedSlope)
         {
            ...
            {
               passFilters = false;
               rejectReason = StringFormat("SpeedSlope_Declining_%.2f<%.2f", g_lastSpeedSlope, MinSpeedSlope);
               if(EnableDebugMode)
                  Print("❌ Filter FAIL: Speed Slope ", g_lastSpeedSlope, " < ", MinSpeedSlope, " (BUY needs positive slope)");
            }
            else if(signal < 0 && g_lastSpeedSlope > -MinSpeedSlope)
            {
               passFilters = false;
               rejectReason = StringFormat("SpeedSlope_Rising_%.2f>%.2f", g_lastSpeedSlope, -MinSpeedSlope);
               if(EnableDebugMode)
                  Print("❌ Filter FAIL: Speed Slope ", g_lastSpeedSlope, " > ", -MinSpeedSlope, " (SELL needs negative slope)");
            }
         }
         ...
         {
            if(signal > 0 && g_lastAccelerationSlope < MinAccelerationSlope)
            {
               passFilters = false;
               rejectReason = StringFormat("AccelSlope_Declining_%.2f<%.2f", g_lastAccelerationSlope, MinAccelerationSlope);
               if(EnableDebugMode)
                  Print("❌ Filter FAIL: Accel Slope ", g_lastAccelerationSlope, " < ", MinAccelerationSlope, " (BUY needs positive slope)");
            }
            else if(signal < 0 && g_lastAccelerationSlope > -MinAccelerationSlope)
            {
               passFilters = false;
               rejectReason = StringFormat("AccelSlope_Rising_%.2f>%.2f", g_lastAccelerationSlope, -MinAccelerationSlope);
               if(EnableDebugMode)
                  Print("❌ Filter FAIL: Accel Slope ", g_lastAccelerationSlope, " > ", -MinAccelerationSlope, " (SELL needs negative slope)");
            }
         }
         ...
         {
            if(signal > 0 && g_lastMomentumSlope < MinMomentumSlope)
            {
               passFilters = false;
               rejectReason = StringFormat("MomentumSlope_Declining_%.2f<%.2f", g_lastMomentumSlope, MinMomentumSlope);
               if(EnableDebugMode)
                  Print("❌ Filter FAIL: Momentum Slope ", g_lastMomentumSlope, " < ", MinMomentumSlope, " (BUY needs positive slope)");
            }
            else if(signal < 0 && g_lastMomentumSlope > -MinMomentumSlope)
            {
               passFilters = false;
               rejectReason = StringFormat("MomentumSlope_Rising_%.2f>%.2f", g_lastMomentumSlope, -MinMomentumSlope);
               if(EnableDebugMode)
                  Print("❌ Filter FAIL: Momentum Slope ", g_lastMomentumSlope, " > ", -MinMomentumSlope, " (SELL needs negative slope)");
            }
         }
         ...
         {
            if(signal > 0 && g_lastJerkSlope < MinJerkSlope)
            {
               passFilters = false;
               rejectReason = StringFormat("JerkSlope_Declining_%.2f<%.2f", g_lastJerkSlope, MinJerkSlope);
               if(EnableDebugMode)
                  Print("❌ Filter FAIL: Jerk Slope ", g_lastJerkSlope, " < ", MinJerkSlope, " (BUY needs positive slope)");
            }
            else if(signal < 0 && g_lastJerkSlope > -MinJerkSlope)
            {
               passFilters = false;
               rejectReason = StringFormat("JerkSlope_Rising_%.2f>%.2f", g_lastJerkSlope, -MinJerkSlope);
               if(EnableDebugMode)
                  Print("❌ Filter FAIL: Jerk Slope ", g_lastJerkSlope, " > ", -MinJerkSlope, " (SELL needs negative slope)");
            }
         }
      }
                  ...
                  double quality, double confluence, TRADING_ZONE zone, VOLATILITY_REGIME regime)
{
   ...
   // Add to tracker (with physics score)
   g_tracker.AddTrade(ticket, quality, confluence, g_physics.GetMomentum(), 
                     g_physics.GetEntropy(), g_lastPhysicsScore,
...
void LogCompletedTrade(ClosedTrade &trade)
{
   ...
   log.entryPhysicsScore = trade.entryPhysicsScore;  // NEW: Evidence-based composite score
