//+------------------------------------------------------------------+
//|                       TP_Integrated_EA_Crossover_4_1_3.mq5       |
//|                      TickPhysics Institutional Framework (ITPF)  |
//|             v4.13 - PRODUCTION READY (QA APPROVED)               |
//+------------------------------------------------------------------+
#property copyright "Copyright 2025, QuanAlpha"
#property link      "https://github.com/quanalpha/tickphysics"
#property version   "4.13"
#property description "v4.13 EA with Universal Elite v3.1 Indicator Support - QA APPROVED"

// EA Version Info (for CSV tracking)
#define EA_NAME "TP_Integrated_EA"
#define EA_VERSION "4.13_PRODUCTION"

input int MagicNumber = 400413;                       // EA magic number
input string TradeComment = "TP_Integrated 4_13";      // Trade comment
...
input int         TakeProfitPips          = 200;       // Take profit (pips) - 2:1 R:R
...
input group "🔬 Physics Score Calculation (Multi-Dataset Analysis)"
...
input double               Weight_Acceleration_1H     = 35.0;           // Acceleration weight for 1H (%)
input double               Weight_Speed_1H            = 20.0;           // Speed weight for 1H (%)
input double               Weight_Jerk_1H             = 15.0;           // Jerk weight for 1H (%)
input double               Weight_Momentum_1H         = 12.0;           // Momentum weight for 1H (%)
input double               Weight_Confluence_1H       = 10.0;           // Confluence weight for 1H (%)
input double               Weight_Quality_1H          = 8.0;            // Quality weight for 1H (%)
input double               Weight_Acceleration_5M     = 30.0;           // Acceleration weight for 5M (%)
input double               Weight_Speed_5M            = 18.0;           // Speed weight for 5M (%)
input double               Weight_Jerk_5M             = 15.0;           // Jerk weight for 5M (%)
input double               Weight_Momentum_5M         = 15.0;           // Momentum weight for 5M (%)
input double               Weight_Confluence_5M       = 12.0;           // Confluence weight for 5M (%)
input double               Weight_Quality_5M          = 10.0;           // Quality weight for 5M (%)
...
void LogSignal(int signal, double quality, double confluence, TRADING_ZONE zone, VOLATILITY_REGIME regime)
{
   ...
   entry.physicsScore = CalculatePhysicsScore();  // NEW: Calculate evidence-based physics score
