//+------------------------------------------------------------------+
//|                  TP_Integrated_EA_Crossover_4_2_0_0_Scalper.mq5  |
//|                      TickPhysics Institutional Framework (ITPF)  |
//|             ⚡ ULTRA-FAST 1M SCALPER - 30+ TRADES/DAY ⚡          |
//+------------------------------------------------------------------+

#property copyright "Copyright 2025, QuanAlpha"
#property link      "https://github.com/quanalpha/tickphysics"
#property description "⚡ v4.2.0.0 1M SCALPER - Optimized for NAS100 M1 with 40-pip targets"

// EA Version Info (for CSV tracking)
#define EA_NAME "TP_Integrated_EA_1M_SCALPER"
#define EA_VERSION "4.2.0.0_1M_SCALPER"

input int MagicNumber = 4004201;                        // EA magic number (unique for scalper)
input string TradeComment = "TP_SCALPER_1M";      // Trade comment


// Indicator Version Selection (SCALPER: Uses 1M Scalper Indicator)
enum INDICATOR_VERSION
{
   ...
   INDICATOR_INDICES = 3,   // TickPhysics_Indices_1M_Scalper_v2_1 (SCALPER)
...
//| Input Parameters                                                  |
//+------------------------------------------------------------------+
// === Risk Management (SCALPER: Lower risk, higher frequency) ===
input group "💰 Risk Management"
input double      RiskPercentPerTrade     = 0.5;         // Risk per trade (% of balance) - SCALPER: 0.5%
input double      MaxDailyRisk            = 15.0;        // Max daily risk (% of balance) - SCALPER: 15%
input int         MaxConcurrentTrades     = 3;           // Max concurrent positions - SCALPER: 3
input double      MinRRatio               = 0.5;         // Min reward:risk ratio - SCALPER: 0.5 (60pip SL / 40pip TP)


// === Trade Parameters (SCALPER: Tight 40-pip targets) ===
input group "📊 Trade Parameters (Asset-Adaptive)"
input bool        UseAssetAdaptiveSLTP    = true;            // Enable asset-specific SL/TP defaults
input int         StopLossPips_Forex      = 15;             // SL Forex (pips) - SCALPER
input int         TakeProfitPips_Forex    = 10;             // TP Forex (pips) - SCALPER
input int         StopLossPips_Indices    = 60;             // SL Indices (pips) - SCALPER: 60 for NAS100 M1
input int         TakeProfitPips_Indices  = 40;             // TP Indices (pips) - SCALPER: 40-pip target
input int         StopLossPips_Crypto     = 100;              // SL Crypto (pips) - SCALPER
input int         TakeProfitPips_Crypto   = 60;              // TP Crypto (pips) - SCALPER
input int         StopLossPips_Metal      = 15;             // SL Metals (pips) - SCALPER
input int         TakeProfitPips_Metal    = 10;             // TP Metals (pips) - SCALPER
input bool        UseTrailingStop         = false;           // Enable trailing stop
input int         TrailingStopPips        = 20;              // Trailing stop (pips) - SCALPER: tighter
...
// === Signal Filters (SCALPER: Relaxed for high frequency) ===
input group "🎯 Physics Filters v4.0"
input bool                 UsePhysicsFilters          = true;            // Enable physics filtering
input double               MinQuality                 = 60.0;            // Min physics quality - SCALPER: 60% (relaxed from 90%)
input bool                 AvoidTransitionZone        = false;           // Reject TRANSITION/AVOID zones - SCALPER: DISABLED
input bool                 UseRegimeFilter            = false;           // Filter by volatility regime

// === Spread Filter (SCALPER: CRITICAL for profitability) ===
input group "📊 Spread Filter (Cost Control)"
input bool                 UseSpreadFilter            = true;            // Enable spread filtering - SCALPER: MANDATORY
input double               MaxSpreadPips              = 15.0;             // Max spread allowed (pips) - SCALPER: 15 pips max
input bool                 UseAdaptiveSpread          = false;           // Use ATR-based adaptive spread limit
input double               MaxSpreadATRMultiple       = 0.3;             // Max spread as % of ATR - SCALPER: 30% of ATR

// === Acceleration/Speed/Momentum Filters (SCALPER: Relaxed thresholds) ===
input group "⚡ Advanced Physics Entry Filters"
input bool                 UseAccelerationFilter      = true;            // Enable acceleration threshold
input double               MinAccelerationBuy         =  0.5;           // Min acceleration for BUY - SCALPER: 0.5 (relaxed)
input double               MinAccelerationSell        = -0.5;          // Min acceleration for SELL - SCALPER: -0.5 (relaxed)
input bool                 UseSpeedFilter             = true;           // Enable speed threshold
input double               MinSpeedBuy                = 0.5;            // Min speed for BUY - SCALPER: 0.5 (relaxed)
input double               MinSpeedSell               = -0.5;           // Min speed for SELL - SCALPER: -0.5 (relaxed)
input bool                 UseMomentumFilter          = false;           // Enable momentum threshold - SCALPER: DISABLED
input double               MinMomentumBuy             = 0.5;            // Min momentum for BUY - SCALPER: 0.5 (relaxed)
input double               MinMomentumSell            = -0.5;           // Min momentum for SELL - SCALPER: -0.5 (relaxed)
...
// === v4.2 Multi-Asset Optimizations (SCALPER: Relaxed for frequency) ===
input group "🎯 v4.2 Entry Filters (Multi-Asset Validated)"
input bool                 UsePhysicsScoreFilter      = true;            // Enable physics score threshold
input double               MinPhysicsScore            = 25.0;            // Min physics score - SCALPER: 25 (relaxed from 50)
input bool                 RequireFullConfluence      = false;            // Require 100% confluence - SCALPER: DISABLED
input bool                 PreferIndices              = true;           // Log index vs forex performance separately - SCALPER: YES

// === v4.5 Slope Filters (SCALPER: DISABLED for max frequency) ===
input group "📈 v4.5 Slope Filters (Trend Direction Confirmation)"
input bool                 UseSlopeFilters            = false;            // Enable slope-based filtering - SCALPER: DISABLED
input int                  SlopeLookbackBars          = 2;               // Bars for slope calculation - SCALPER: 2 (faster)
input bool                 UseSpeedSlope              = false;            // Require Speed slope trending in signal direction
input double               MinSpeedSlopeBuy           = 0.3;             // Min Speed slope for BUY - SCALPER: 0.3 (relaxed)
input double               MinSpeedSlopeSell          = -0.3;            // Min Speed slope for SELL - SCALPER: -0.3 (relaxed)
input bool                 UseAccelerationSlope       = false;           // Require Acceleration slope confirmation
input double               MinAccelerationSlopeBuy    = 0.3;             // Min Acceleration slope for BUY - SCALPER: 0.3 (relaxed)
input double               MinAccelerationSlopeSell   = -0.3;            // Min Acceleration slope for SELL - SCALPER: -0.3 (relaxed)
input bool                 UseConfluenceSlope         = false;           // Require Confluence trending upward
input double               MinConfluenceSlope         = 0.5;             // Min Confluence slope - SCALPER: 0.5 (relaxed)
input bool                 UseMomentumSlope           = false;           // Require Momentum slope confirmation
input double               MinMomentumSlopeBuy        = 0.3;             // Min Momentum slope for BUY - SCALPER: 0.3 (relaxed)
input double               MinMomentumSlopeSell       = -0.3;            // Min Momentum slope for SELL - SCALPER: -0.3 (relaxed)
input bool                 UseJerkSlope               = false;           // Use Jerk slope - SCALPER: DISABLED (too noisy for 1M)
input double               MinJerkSlopeBuy            = 0.3;             // Min Jerk slope for BUY - SCALPER: 0.3 (relaxed)
input double               MinJerkSlopeSell           = -0.3;            // Min Jerk slope for SELL - SCALPER: -0.3 (relaxed)
...
string GetIndicatorName(INDICATOR_VERSION version)
{
   ...
   if(version == INDICATOR_AUTO)
   {
      ...
      {
         Print("📊 ⚡ SCALPER: Auto-detected INDICES symbol: Using TickPhysics_Indices_1M_Scalper_v2_1");
         return "TickPhysics_Indices_1M_Scalper_v2_1";
      }
   ...
   switch(version)
   {
      ...
      case INDICATOR_INDICES:
         Print("📊 ⚡ SCALPER: Manual selection: Using TickPhysics_Indices_1M_Scalper_v2_1");
         return "TickPhysics_Indices_1M_Scalper_v2_1";
                  ...
                  double quality, double confluence, TRADING_ZONE zone, VOLATILITY_REGIME regime)
{
   ...
   // Crypto-specific pip calculation fix
   // For crypto like BITCOIN (2 digits, point=0.01), treat "pips" as full dollars
   // For forex (4-5 digits), use standard pip calculation
   double pipMultiplier = 1.0;
   int assetClass = (int)g_riskManager.GetAssetClass();
   
   if(assetClass == 1)  // CRYPTO
   {
      // For crypto: 1 "pip" input = $1.00 actual distance
      // BITCOIN: 2 digits, point=0.01, so multiply pips by 100 to get dollars
      // Example: 400 pips input * 0.01 point * 100 = $400.00 distance
      pipMultiplier = (digits == 2) ? 100.0 : 10.0;  // 100 for BTC/ETH, 10 for some others
      
      if(EnableDebugMode)
         Print("💱 CRYPTO pip calculation: digits=", digits, " pipMultiplier=", pipMultiplier);
   }
   else if(digits == 3 || digits == 5)
   {
      // Standard forex 5-digit broker adjustment
      pipMultiplier = 10.0;
   }
