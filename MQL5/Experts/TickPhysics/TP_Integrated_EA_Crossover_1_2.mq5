//+------------------------------------------------------------------+
//|                            TP_Integrated_EA_Crossover_1_2.mq5    |
//|                      TickPhysics Institutional Framework (ITPF)  |
//|                          Full Integration EA - All 4 Libraries   |
//+------------------------------------------------------------------+
#property copyright "Copyright 2025, QuanAlpha"
#property link      "https://github.com/quanalpha/tickphysics"
#property version   "1.0"
#property description "Production EA integrating Physics, Risk, Tracker, and Logger"
...
// === Signal Filters ===
input group "🎯 Physics Filters (Disable for MA-only baseline)"
input bool UsePhysicsFilters = false;          // Enable physics quality filtering
input double MinQuality = 65.0;                // Min physics quality (logged always)
input double MinConfluence = 70.0;             // Min confluence (logged always)
input bool UseZoneFilter = false;              // Filter by trading zone
...
int OnInit()
{
   ...
   loggerConfig.signalLogFile = "TP_Integrated_Signals_" + _Symbol + ".csv";
   loggerConfig.tradeLogFile = "TP_Integrated_Trades_" + _Symbol + ".csv";
   ...
   Print("   Min Quality: ", MinQuality, " (", UsePhysicsFilters ? "filtering" : "logged only", ")");
   Print("   Min Confluence: ", MinConfluence, " (", UsePhysicsFilters ? "filtering" : "logged only", ")");
...
int GenerateMASignal()
{
   ...
   if(CopyBuffer(g_maFastHandle, 0, 0, 2, maFast) < 2) return 0;
   if(CopyBuffer(g_maSlowHandle, 0, 0, 2, maSlow) < 2) return 0;
   
   // BUY Signal: Fast MA crosses above Slow MA
   if(maFast[1] <= maSlow[1] && maFast[0] > maSlow[0])
   {
      if(EnableDebugMode)
         Print("🟢 MA BUY SIGNAL: Fast(", maFast[0], ") crossed above Slow(", maSlow[0], ")");
      return 1;  // BUY
   }
   ...
   {
      if(EnableDebugMode)
         Print("🔴 MA SELL SIGNAL: Fast(", maFast[0], ") crossed below Slow(", maSlow[0], ")");
      return -1;  // SELL
   }
...
void OnNewBar()
{
   ...
   if(UsePhysicsFilters)
   {
      ...
      else if(UseZoneFilter && zone == ZONE_AVOID)
      {
         rejectReason = "Avoid zone";
         passFilters = false;
      }
      else if(UseRegimeFilter && regime == REGIME_HIGH)
      {
         rejectReason = "High volatility regime";
         passFilters = false;
      }
      ...
      {
         if(EnableDebugMode)
            Print("🚫 Signal rejected: ", rejectReason, " (Q=", quality, ", C=", confluence, ")");
         return;
      }
   }
   else
   ...
   if(closedCount > 0)
   {
      Sleep(100);  // 100ms delay to ensure position is fully closed
      if(EnableDebugMode)
         Print("⏱️  Waited 100ms for position close to process");
   }
   
   // 6. Check if we have any existing position in the signal direction
   // (Don't open duplicate positions)
   if(HasPositionInDirection(signal))
   {
      if(EnableDebugMode)
         Print("ℹ️  Already have position in ", (signal > 0 ? "BUY" : "SELL"), " direction, skipping entry");
      return;
   }
...
void DrawMAIndicators()
{
   ...
   {
      // Set Fast MA properties (Blue, width 2)
      int subwindow = 0;
      string short_name = "";
      ChartIndicatorName(0, subwindow, ChartIndicatorsTotal(0, subwindow) - 1);
