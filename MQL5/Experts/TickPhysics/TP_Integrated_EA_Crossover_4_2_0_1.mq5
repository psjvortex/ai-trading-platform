//+------------------------------------------------------------------+
//|                    TP_Integrated_EA_Crossover_4_2_0_1.mq5        |
...
#property description "v4.2.0.1 EA with Slope Analysis + Universal Elite v3.1 Indicator Support - QA APPROVED"

// EA Version Info (for CSV tracking)
#define EA_NAME "TP_Integrated_EA"
#define EA_VERSION "4.2.0.1_SLOPE"

input int MagicNumber = 4004201;                        // EA magic number
input string TradeComment = "TP_Integrated 4_2_0_1";      // Trade comment
...
input int         StopLossPips_Indices    = 500;             // SL Indices (pips) - 350 points (reasonable for NAS100/US30)
input int         TakeProfitPips_Indices  = 500;             // TP Indices (pips)
...
input double               MinAccelerationBuy         =  1000.0;           // Min acceleration for BUY (positive)
input double               MinAccelerationSell        = -1000.0;          // Min acceleration for SELL (negative value)
input bool                 UseSpeedFilter             = true;           // Enable speed threshold
input double               MinSpeedBuy                = 1000.0;            // Min speed for BUY (positive)
input double               MinSpeedSell               = -1000.0;           // Min speed for SELL (negative value)
input bool                 UseMomentumFilter          = true;           // Enable momentum threshold
input double               MinMomentumBuy             = 1000.0;            // Min momentum for BUY (positive)
input double               MinMomentumSell            = -1000.0;           // Min momentum for SELL (negative value)
...
input double               MinPhysicsScore            = 50.0;            // Min physics score (Q3 threshold: 55-85 optimal)
...
input double               MinSpeedSlopeBuy           = 1000.0;             // Min Speed slope for BUY (positive, units/bar)
input double               MinSpeedSlopeSell          = -1000.0;            // Min Speed slope for SELL (negative, units/bar)
input bool                 UseAccelerationSlope       = true;           // Require Acceleration slope confirmation
input double               MinAccelerationSlopeBuy    = 1000.0;             // Min Acceleration slope for BUY (positive, units/bar)
input double               MinAccelerationSlopeSell   = -1000.0;            // Min Acceleration slope for SELL (negative, units/bar)
input bool                 UseConfluenceSlope         = true;           // Require Confluence trending upward
input double               MinConfluenceSlope         = 1000.0;             // Min Confluence slope (percent/bar)
input bool                 UseMomentumSlope           = true;           // Require Momentum slope confirmation
input double               MinMomentumSlopeBuy        = 1000.0;             // Min Momentum slope for BUY (positive, units/bar)
input double               MinMomentumSlopeSell       = -1000.0;            // Min Momentum slope for SELL (negative, units/bar)
input bool                 UseJerkSlope               = true;           // Use Jerk slope (advanced, may be noisy)
input double               MinJerkSlopeBuy            = 1000.0;             // Min Jerk slope for BUY (positive, units/bar)
input double               MinJerkSlopeSell           = -1000.0;            // Min Jerk slope for SELL (negative, units/bar)
