// INSTRUCTION TO COPILOT: Generate the FINAL production version of this EA and name it "Falcon Heavy Ludicrous Mode™ v5.0 Institutional"
// Use EVERYTHING that made money live on November 18-19 2025 with the nuclear settings (MinQuality = 95, RequireFullConfluence = true, all physics & slope thresholds = 100.0)
// Add a single master switch at the top: input bool LudicrousMode = true; → when true, it forces those exact nuclear filters (make them non-configurable in Ludicrous mode)
// COMPLETELY REPLACE the old pip-based SL/TP system with UNIVERSAL DOLLAR-BASED EXITS:
//   - input double DollarRiskPerTrade = 200.0;
//   - input double RewardRatio = 3.0;  // default 1:3
//   - Calculate lot size using g_riskManager.CalculateLotSizeFromMoney(DollarRiskPerTrade)
//   - Calculate exact SL and TP prices using g_riskManager.MoneyToPriceDistance() so that every trade risks exactly DollarRiskPerTrade and targets exactly DollarRiskPerTrade * RewardRatio, on EVERY symbol (indices, forex, gold, crypto, JPY pairs — everything)
// Add institutional guardrails:
//   - Max TOTAL positions across entire account = 3 (not per symbol)
//   - Correlation kill-switch: do not allow simultaneous same-direction positions on NAS100, US30, US500, SPX500, US2000
//   - Daily hard loss cap in dollars: input double MaxDailyLoss = 1000.0; → if AccountInfoDouble(ACCOUNT_PROFIT) today <= -MaxDailyLoss → no new trades until next day
//   - Regime-based lot scaling: in LOW regime → multiply lots by 0.2, NORMAL → 0.6, HIGH → 1.0 (only when Quality >= 95)
// Keep the asset-adaptive SL/TP toggle as a fallback, but when LudicrousMode = true, force dollar exits only
// Logger is already perfect in this file — leave it exactly as-is (timestamp + LIVE/MTBacktest detection)
// Remove ALL old per-asset pip inputs (StopLossPips_Forex etc.) — they are dead to us now
// Change EA name and version to:
//   #define EA_NAME    "FalconHeavy_Ludicrous"
//   #define EA_VERSION "v5.0_Institutional"
//   Indicator shortname in HUD: "Falcon Heavy Ludicrous v5.0"
// Add a big beautiful header comment at the top with rocket emojis and "The Final Form – November 19, 2025"
// Make sure the code is clean, heavily commented, and ready for immediate live deployment on 50+ symbols
// This is the version that turned $10k → $14k in <3 hours live on Nov 18 2025 and +1,240% in backtest across 8 symbols — treat it with respect

// NOW GENERATE THE FULL FINAL .MQ5 CODE BELOW THIS LINE

//+------------------------------------------------------------------+
//|                       TP_Integrated_EA_Crossover_4_1_9_7.mq5     |
...
#property description "v4.1.9.7 EA with Slope Analysis + Universal Elite v3.1 Indicator Support - QA APPROVED"

// EA Version Info (for CSV tracking)
#define EA_NAME "TP_Integrated_EA"
#define EA_VERSION "4.1.9.7_SLOPE_BT"

input int MagicNumber = 4004197;                        // EA magic number
input string TradeComment = "TP_Integrated 4_1_9_7";      // Trade comment
...
input int         StopLossPips_Forex      = 10;             // SL Forex (pips) - 90 points on 5-digit
input int         TakeProfitPips_Forex    = 5;             // TP Forex (pips) - 30 points
input int         StopLossPips_Indices    = 600;             // SL Indices (pips) - 350 points (reasonable for NAS100/US30)
input int         TakeProfitPips_Indices  = 200;             // TP Indices (pips)
input int         StopLossPips_Crypto     = 300;              // SL Crypto (pips) - $500 for BTCUSD ($10/pip)
input int         TakeProfitPips_Crypto   = 100;              // TP Crypto (pips) - $250 for BTCUSD
input int         StopLossPips_Metal      = 300;             // SL Metals (pips) - $8 for XAUUSD
input int         TakeProfitPips_Metal    = 100;             // TP Metals (pips)
input bool        UseTrailingStop         = false;           // Enable trailing stop
input int         TrailingStopPips        = 30;              // Trailing stop (pips)

// === Entry System Selection ===
input group "📊 Entry Logic"
input INDICATOR_VERSION    IndicatorVersion           = INDICATOR_CRYPTO;  // Indicator version to use (UNIVERSAL recommended)
...
input double               MinQuality                 = 95.0;            // Min physics quality (v4.0 threshold)
...
input double               MinAccelerationSell        = -100.0;          // Min acceleration for SELL (negative value)
input bool                 UseSpeedFilter             = true;           // Enable speed threshold
input double               MinSpeedBuy                = 100.0;            // Min speed for BUY (positive)
input double               MinSpeedSell               = -100.0;           // Min speed for SELL (negative value)
input bool                 UseMomentumFilter          = true;           // Enable momentum threshold
input double               MinMomentumBuy             = 100.0;            // Min momentum for BUY (positive)
input double               MinMomentumSell            = -100.0;           // Min momentum for SELL (negative value)
...
input double               MinPhysicsScore            = 90.0;            // Min physics score (Q3 threshold: 55-85 optimal)
...
input double               MinSpeedSlopeBuy           = 100.0;             // Min Speed slope for BUY (positive, units/bar)
input double               MinSpeedSlopeSell          = -100.0;            // Min Speed slope for SELL (negative, units/bar)
input bool                 UseAccelerationSlope       = true;           // Require Acceleration slope confirmation
input double               MinAccelerationSlopeBuy    = 100.0;             // Min Acceleration slope for BUY (positive, units/bar)
input double               MinAccelerationSlopeSell   = -100.0;            // Min Acceleration slope for SELL (negative, units/bar)
...
input double               MinMomentumSlopeBuy        = 100.0;             // Min Momentum slope for BUY (positive, units/bar)
input double               MinMomentumSlopeSell       = -100.0;            // Min Momentum slope for SELL (negative, units/bar)
input bool                 UseJerkSlope               = true;           // Use Jerk slope (advanced, may be noisy)
input double               MinJerkSlopeBuy            = 100.0;             // Min Jerk slope for BUY (positive, units/bar)
input double               MinJerkSlopeSell           = -100.0;            // Min Jerk slope for SELL (negative, units/bar)
...
input bool                 EnableDebugMode            = false;            // Verbose logging
...
input bool                 ShowDashboard              = false;            // Show on-chart dashboard
...
string GetIndicatorName(INDICATOR_VERSION version)
{
   ...
   if(version == INDICATOR_AUTO)
   {
      ...
      // Metals detection (XAUUSD, GOLD, SILVER, etc.)
