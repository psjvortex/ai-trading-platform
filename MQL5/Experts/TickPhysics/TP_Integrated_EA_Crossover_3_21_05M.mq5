...
//|         Hybrid: MA Reversal Exits + Momentum Filter              |
//+------------------------------------------------------------------+
#property copyright "Copyright 2025, QuanAlpha"
#property link      "https://github.com/quanalpha/tickphysics"
#property version   "3.21"
#property description "v3.21_05M: HYBRID APPROACH - Best of v3.1 + v3.2"
#property description "Keep: Zone/Regime/Time filters + Momentum filter (MinMomentum -346.58)"
#property description "Remove: SL/TP (use MA reversal exits like v3.1 - worked better!)"
#property description "Reason: v3.2 SL too tight (63.8% hit rate), TP too high (12.6% hit rate)"

// EA Version Info (for CSV tracking)
#define EA_NAME "TP_Integrated_EA"
#define EA_VERSION "3.21_05M"

input int MagicNumber = 300321;                // EA magic number (v3.21 = 300321)
input string TradeComment = "TP_Integrated 3_21_05M";   // Trade comment
...
// === Trade Parameters - v3.21 NO SL/TP (MA REVERSAL EXITS) ===
input group "📊 Trade Parameters - v3.21 MA REVERSAL EXITS"
input int StopLossPips = 0;                    // Stop loss (pips) - v3.21 DISABLED (use MA reversals)
input int TakeProfitPips = 0;                  // Take profit (pips) - v3.21 DISABLED (use MA reversals)
...
// === Signal Filters - v3.21 HYBRID (v3.1 + v3.2 Momentum) ===
input group "🎯 Physics Filters - v3.21 HYBRID OPTIMIZATION"
input bool UsePhysicsFilters = true;            // Enable physics filtering - v3.21 ENABLED
...
input double MinMomentum = -346.58;            // Min momentum - v3.2 KEEP! (strongest separator, 55.86 gap)
...
{
   Print("═══════════════════════════════════════════════════════");
   Print("🚀 TickPhysics Integrated EA v3.21_05M - Initializing");
   Print("═══════════════════════════════════════════════════════");
   ...
   Print("");
   Print("✅ ALL SYSTEMS READY - v3.21_05M HYBRID OPTIMIZATION!");
   Print("═══════════════════════════════════════════════════════");
   Print("📋 Configuration:");
   Print("   EA Version: v3.21_05M (Hybrid: MA Reversals + Momentum Filter)");
   Print("   🎯 v3.21 STRATEGY: Best of v3.1 (MA exits) + v3.2 (Momentum filter)");
   Print("   Entry System: ", UsePhysicsEntry ? "PHYSICS" : (UseMAEntry ? "MA CROSSOVER" : "NONE"));
   if(UseMAEntry)
      Print("   MA Periods: Fast=", MA_Fast, ", Slow=", MA_Slow);
   Print("");
   Print("   💥 v3.21 KEY FEATURES:");
   Print("   → Exit Method: MA REVERSAL (like v3.1 - 38.5% WR vs v3.2 SL/TP 22.5% WR)");
   Print("   → Stop Loss: DISABLED (0 pips) - MA crossover exits only");
   Print("   → Take Profit: DISABLED (0 pips) - MA crossover exits only");
   Print("   → MinMomentum: ", MinMomentum, " (v3.2 filter - 30% trade reduction)");
   Print("");
   ...
   Print("   CSV Files: v3.21_05M (Trades & Signals)");
   Print("");
   Print("   📊 Expected Performance (Hybrid Approach):");
   Print("   → Estimated Trades: ~373 (v3.2 Momentum filter reduction)");
   Print("   → Target WR: 35-45% (v3.1 MA exits 38.5% + v3.2 Momentum boost)");
   Print("   → Target PF: 1.4-1.8");
   Print("   → Net Profit: Better than v3.2's $402 with higher WR");
   Print("═══════════════════════════════════════════════════════");
...
{
   Print("═══════════════════════════════════════════════════════");
   Print("🛑 TickPhysics Integrated EA v3.21_05M - Shutting Down");
   Print("   Reason: ", reason);
   Print("═══════════════════════════════════════════════════════");
...
void OnNewBar()
{
   ...
   // 4. Apply v3.21 physics filters (v3.1 + v3.2 Momentum filter)
   string rejectReason = "";
   ...
   if(UsePhysicsFilters)
   {
      ...
      // v3.2 Momentum filter (CRITICAL - strongest separator)
      else if(momentum < MinMomentum)
      {
         rejectReason = StringFormat("Momentum %.2f < %.2f (v3.2 FILTER)", momentum, MinMomentum);
         passFilters = false;
      }
      ...
      {
         if(EnableDebugMode)
            Print("🚫 v3.21 Signal rejected: ", rejectReason, " (Zone=", g_physics.GetZoneName(zone), ", Regime=", g_physics.GetRegimeName(regime), ", Momentum=", momentum, ")");
         return;
      }
      else if(EnableDebugMode)
      {
         Print("✅ v3.21 Signal ACCEPTED: Zone=", g_physics.GetZoneName(zone), ", Regime=", g_physics.GetRegimeName(regime), ", Momentum=", momentum);
      }
   }
   ...
   // 9. Calculate position size (NO SL/TP in v3.21 - use minimal lot size)
   ...
   if(EnableDebugMode)
      Print("📏 Position size: ", lots, " lots (minimal - no SL/TP, MA reversal exits)");
   ...
   // 10. Execute trade (NO SL/TP - MA reversal exits only)
   if(EnableDebugMode)
      Print("📊 v3.21 Signal confirmed: ", (signal > 0 ? "BUY" : "SELL"), " | Momentum: ", momentum, " | Closed opposite: ", closedCount);
   
   Print("🚀 v3.21_05M EXECUTING TRADE NOW (MA REVERSAL + MOMENTUM FILTER)...");
   ExecuteTrade(signal, lots, price, 0, 0, quality, confluence, momentum, zone, regime);
}
                  ...
                  double quality, double confluence, double momentum, TRADING_ZONE zone, VOLATILITY_REGIME regime)
{
   ...
   // v3.21: NO SL/TP - MA reversal exits only
   if(signal > 0)  // BUY
   {
      Print("🟢 Opening BUY: ", lots, " lots @ ", price, " | Exit: MA REVERSAL (no SL/TP)");
      success = g_trade.Buy(lots, _Symbol, price, 0, 0, TradeComment);
   ...
   {
      Print("🔴 Opening SELL: ", lots, " lots @ ", price, " | Exit: MA REVERSAL (no SL/TP)");
      success = g_trade.Sell(lots, _Symbol, price, 0, 0, TradeComment);
      ticket = g_trade.ResultOrder();
   ...
   Print("✅ v3.21_05M Position opened: #", ticket);
   Print("   💥 v3.21 Hybrid: MA Reversals (v3.1 38.5% WR) + Momentum Filter (v3.2)");
