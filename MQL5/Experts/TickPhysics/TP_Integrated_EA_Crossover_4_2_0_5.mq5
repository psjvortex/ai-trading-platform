//+------------------------------------------------------------------+
//|                    TP_Integrated_EA_Crossover_4_2_0_5.mq5        |
...
#property description "v4.2.0.5 EA with Complete Data Integrity + ConfluenceSlope + Spread + Entropy - ML-READY"

// EA Version Info (for CSV tracking)
#define EA_NAME "TP_Integrated_EA"
#define EA_VERSION "4.2.0.5_COMPLETE"

input int MagicNumber = 4004205;                        // EA magic number
input string TradeComment = "TP_Integrated 4_2_0_5";      // Trade comment
...
#include <TickPhysics_Main/TP_Physics_Indicator.mqh>
#include <TickPhysics_Main/TP_Risk_Manager.mqh>
#include <TickPhysics_Main/TP_Trade_Tracker.mqh>
#include <TickPhysics_Main/TP_CSV_Logger.mqh>
...
input double               MinQuality                 = 90.0;            // Min physics quality (v4.0 threshold)
...
double CalculateRegressionSlope(int lookback, int metricType)
{
   ...
   for(int i = 0; i < lookback; i++)
   {
      ...
      switch(metricType)
      {
         ...
         case 3: y = g_physics.GetConfluence(barIdx); break;
...
int OnInit()
{
   ...
   {
   Print(StringFormat("   → Quality Filter: >= %d%%", MinQuality));
   ...
   if(UsePhysicsScoreFilter)
   Print(StringFormat("      Min Score >= %d (Q3 threshold, +16%% win rate)", MinPhysicsScore));
   ...
   Print(StringFormat("      Lookback Bars: %d", SlopeLookbackBars));
      ...
      if(UseConfluenceSlope)
         Print(StringFormat("      → Confluence Slope >= %s (alignment strengthening)", DoubleToString(MinConfluenceSlope, 2)));
      if(UseMomentumSlope)
      {
   ...
   if(UseIsolatedTesting)
   {
      ...
      if(IsolatedQualityOnly)
      {
         if(quality >= MinQuality && speed > 0)
         {
            ...
            if(EnableDebugMode)
               Print(StringFormat("🧪 ISOLATED QUALITY BUY: Quality=%s (threshold=%d)", DoubleToString(quality, 2), MinQuality));
         }
         else if(quality >= MinQuality && speed < 0)
         {
            signal = -1;  // SELL (if downtrend based on speed)
            if(EnableDebugMode)
               Print(StringFormat("🧪 ISOLATED QUALITY SELL: Quality=%s (threshold=%d)", DoubleToString(quality, 2), MinQuality));
         }
         return signal;
      }
   }
...
void OnNewBar()
{
   ...
   if((UseMAEntry || UsePhysicsEntry) && UsePhysicsFilters && !BypassAllFilters)
   {
      ...
      if(quality < MinQuality)
      {
         ...
         if(EnableDebugMode)
            Print(StringFormat("❌ Filter FAIL: Quality %s < %d", DoubleToString(quality, 2), MinQuality));
      }
      ...
      if(AvoidTransitionZone && (zone == ZONE_TRANSITION || zone == ZONE_AVOID))
      {
         ...
         if(EnableDebugMode)
            Print(StringFormat("❌ Filter FAIL: %s detected", rejectReason));
      }
      ...
      if(UsePhysicsScoreFilter && passFilters)
      {
         if(g_lastPhysicsScore < MinPhysicsScore)
         {
            ...
            if(EnableDebugMode)
               Print(StringFormat("❌ Filter FAIL: Physics Score %s < %d (Q3 threshold)", DoubleToString(g_lastPhysicsScore, 2), MinPhysicsScore));
         }
      }
      ...
      if(UseSlopeFilters && passFilters)
      {
         ...
         if(UseConfluenceSlope && passFilters)
         {
            if(g_lastConfluenceSlope < MinConfluenceSlope)
            {
               ...
               if(EnableDebugMode)
                  Print(StringFormat("❌ Filter FAIL: Confluence Slope %s < %s (needs upward trend)", DoubleToString(g_lastConfluenceSlope, 2), DoubleToString(MinConfluenceSlope, 2)));
            }
         }
