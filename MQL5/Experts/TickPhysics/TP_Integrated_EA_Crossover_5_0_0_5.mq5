//+------------------------------------------------------------------+
//|                    TP_Integrated_EA_Crossover_5_0_0_5.mq5        |
...
// #property description "v5.0.0.5 EA - CEILING FILTERS - Anti-Spike Protection"

// EA Version Info (for CSV tracking)
#define EA_NAME "TP_Integrated_EA"
#define EA_VERSION "5.0.0.5"
...
input double               MinQualitySell             = 60.0;            // Min physics quality for SELL
...
input double               MaxAccelerationBuy         = 99999.0;         // Max acceleration for BUY (anti-spike ceiling)
input double               MinAccelerationSell        = -1.0;            // Min acceleration for SELL (negative value)
input double               MaxAccelerationSell        = -99999.0;        // Max acceleration for SELL (anti-spike ceiling, negative)
input bool                 UseSpeedFilter             = true;            // Enable speed threshold
input double               MinSpeedBuy                = 1.0;             // Min speed for BUY (positive)
input double               MaxSpeedBuy                = 99999.0;         // Max speed for BUY (anti-spike ceiling)
input double               MinSpeedSell               = -1.0;            // Min speed for SELL (negative value)
input double               MaxSpeedSell               = -99999.0;        // Max speed for SELL (anti-spike ceiling, negative)
input bool                 UseMomentumFilter          = true;            // Enable momentum threshold
input double               MinMomentumBuy             = 1.0;             // Min momentum for BUY (positive)
input double               MaxMomentumBuy             = 99999.0;         // Max momentum for BUY (anti-spike ceiling)
input double               MinMomentumSell            = -1.0;            // Min momentum for SELL (negative value)
input double               MaxMomentumSell            = -99999.0;        // Max momentum for SELL (anti-spike ceiling, negative)
...
input double               MinPhysicsScoreBuy         = 30.0;            // Min physics score for BUY
input double               MinPhysicsScoreSell        = 30.0;            // Min physics score for SELL
...
input double               MaxSpeedSlopeBuy           = 99999.0;         // Max Speed slope for BUY (anti-spike ceiling)
input double               MinSpeedSlopeSell          = -1.0;            // Min Speed slope for SELL (negative, units/bar)
input double               MaxSpeedSlopeSell          = -99999.0;        // Max Speed slope for SELL (anti-spike ceiling, negative)
...
input double               MaxAccelerationSlopeBuy    = 99999.0;         // Max Acceleration slope for BUY (anti-spike ceiling)
input double               MinAccelerationSlopeSell   = -1.0;            // Min Acceleration slope for SELL (negative, units/bar)
input double               MaxAccelerationSlopeSell   = -99999.0;        // Max Acceleration slope for SELL (anti-spike ceiling, negative)
...
input double               MaxConfluenceSlopeBuy      = 99999.0;         // Max Confluence slope for BUY (anti-spike ceiling)
input double               MinConfluenceSlopeSell     = 1.0;             // Min Confluence slope for SELL (percent/bar)
input double               MaxConfluenceSlopeSell     = 99999.0;         // Max Confluence slope for SELL (anti-spike ceiling)
...
input double               MaxMomentumSlopeBuy        = 99999.0;         // Max Momentum slope for BUY (anti-spike ceiling)
input double               MinMomentumSlopeSell       = -1.0;            // Min Momentum slope for SELL (negative, units/bar)
input double               MaxMomentumSlopeSell       = -99999.0;        // Max Momentum slope for SELL (anti-spike ceiling, negative)
...
input double               MaxJerkSlopeBuy            = 99999.0;         // Max Jerk slope for BUY (anti-spike ceiling)
input double               MinJerkSlopeSell           = -1.0;            // Min Jerk slope for SELL (negative, units/bar)
input double               MaxJerkSlopeSell           = -99999.0;        // Max Jerk slope for SELL (anti-spike ceiling, negative)
...
// === Time Segment Filters (CST-Based, matches CSV data model) ===
// Segment naming: IN_CST_Day_OP_01, IN_Segment_15M_OP_01, etc.
// All segments are 1-indexed (first segment = 1, not 0)
// CST = MT5 broker time - 8 hours

input group "📅 Day of Week Filter (IN_CST_Day_OP_01)"
...
input group "⏱️ 15-Min Segment Filter (IN_Segment_15M_OP_01: 15-001 to 15-096)"
input bool                 UseSegment15M              = false;           // Enable 15-minute segment filter
input int                  Segment15M_Min             = 1;               // Min segment (1-96), 1=00:00-00:14
input int                  Segment15M_Max             = 96;              // Max segment (1-96), 96=23:45-23:59

input group "⏱️ 30-Min Segment Filter (IN_Segment_30M_OP_01: 30-001 to 30-048)"
input bool                 UseSegment30M              = false;           // Enable 30-minute segment filter
input int                  Segment30M_Min             = 1;               // Min segment (1-48), 1=00:00-00:29
input int                  Segment30M_Max             = 48;              // Max segment (1-48), 48=23:30-23:59

input group "⏱️ 1-Hour Segment Filter (IN_Segment_01H_OP_01: 1h-001 to 1h-024)"
input bool                 UseSegment01H              = false;           // Enable 1-hour segment filter
input int                  Segment01H_Min             = 1;               // Min segment (1-24), 1=00:00-00:59
input int                  Segment01H_Max             = 24;              // Max segment (1-24), 24=23:00-23:59

input group "⏱️ 2-Hour Segment Filter (IN_Segment_02H_OP_01: 2h-001 to 2h-012)"
input bool                 UseSegment02H              = false;           // Enable 2-hour segment filter
input int                  Segment02H_Min             = 1;               // Min segment (1-12), 1=00:00-01:59
input int                  Segment02H_Max             = 12;              // Max segment (1-12), 12=22:00-23:59

input group "⏱️ 3-Hour Segment Filter (IN_Segment_03H_OP_01: 3h-001 to 3h-008)"
input bool                 UseSegment03H              = false;           // Enable 3-hour segment filter
input int                  Segment03H_Min             = 1;               // Min segment (1-8), 1=00:00-02:59
input int                  Segment03H_Max             = 8;               // Max segment (1-8), 8=21:00-23:59

input group "⏱️ 4-Hour Segment Filter (IN_Segment_04H_OP_01: 4h-001 to 4h-006)"
input bool                 UseSegment04H              = false;           // Enable 4-hour segment filter
input int                  Segment04H_Min             = 1;               // Min segment (1-6), 1=00:00-03:59
input int                  Segment04H_Max             = 6;               // Max segment (1-6), 6=20:00-23:59
...
//+------------------------------------------------------------------+
//| CST Time Segment Structure (matches CSV data model)              |
//| CST = MT5 broker time - 8 hours                                  |
//+------------------------------------------------------------------+
struct CSTTimeSegments
{
   int      dayOfWeek;      // 0=Sunday, 1=Monday, ..., 6=Saturday
   string   dayName;        // "Sunday", "Monday", etc.
   int      segment15M;     // 1-96 (matches IN_Segment_15M_OP_01)
   int      segment30M;     // 1-48 (matches IN_Segment_30M_OP_01)
   int      segment01H;     // 1-24 (matches IN_Segment_01H_OP_01)
   int      segment02H;     // 1-12 (matches IN_Segment_02H_OP_01)
   int      segment03H;     // 1-8  (matches IN_Segment_03H_OP_01)
   int      segment04H;     // 1-6  (matches IN_Segment_04H_OP_01)
   int      cstHour;        // 0-23 CST hour
   int      cstMinute;      // 0-59 CST minute
};

//+------------------------------------------------------------------+
//| Calculate CST Time Segments from current MT5 time                |
//| MT5 broker time is typically GMT+2, CST is GMT-6 (8 hour diff)   |
//+------------------------------------------------------------------+
CSTTimeSegments CalculateCSTSegments()
{
   CSTTimeSegments seg;
   
   // Get current MT5 broker time
   datetime mt5Time = TimeCurrent();
   
   // Convert to CST (subtract 8 hours)
   datetime cstTime = mt5Time - (8 * 3600);
   ...
   seg.dayName = dayNames[seg.dayOfWeek];
   
   // CST hour and minute
   seg.cstHour = cstDt.hour;
   seg.cstMinute = cstDt.min;
   
   // Calculate time segments (1-indexed, matching CSV data model)
   // Segment formula: hour * segments_per_hour + floor(minute / segment_size) + 1
   seg.segment15M = seg.cstHour * 4 + (seg.cstMinute / 15) + 1;      // 1-96
   seg.segment30M = seg.cstHour * 2 + (seg.cstMinute / 30) + 1;      // 1-48
   seg.segment01H = seg.cstHour + 1;                                  // 1-24
   seg.segment02H = (seg.cstHour / 2) + 1;                           // 1-12
   seg.segment03H = (seg.cstHour / 3) + 1;                           // 1-8
   seg.segment04H = (seg.cstHour / 4) + 1;                           // 1-6
   
   return seg;
}

//+------------------------------------------------------------------+
//| Check if current time passes all time segment filters            |
//| Returns true if trade is allowed, false if blocked               |
//+------------------------------------------------------------------+
bool CheckTimeSegmentFilters(string &rejectReason)
{
   CSTTimeSegments seg = CalculateCSTSegments();
   
   // Day of Week Filter (IN_CST_Day_OP_01)
   if(UseDayFilter)
   {
      bool allowedDay = false;
      switch(seg.dayOfWeek)
      {
         case 0: allowedDay = AllowSunday; break;
         case 1: allowedDay = AllowMonday; break;
         case 2: allowedDay = AllowTuesday; break;
         case 3: allowedDay = AllowWednesday; break;
         case 4: allowedDay = AllowThursday; break;
         case 5: allowedDay = AllowFriday; break;
         case 6: allowedDay = AllowSaturday; break;
      }
      
      if(!allowedDay)
      {
         rejectReason = StringFormat("DAY_FILTER_%s", seg.dayName);
         if(EnableDebugMode)
            Print(StringFormat("📅 Time Filter BLOCKED: %s not allowed (CST)", seg.dayName));
         return false;
      }
   }
   
   // 15-Minute Segment Filter (IN_Segment_15M_OP_01: 15-001 to 15-096)
   if(UseSegment15M)
   {
      if(seg.segment15M < Segment15M_Min || seg.segment15M > Segment15M_Max)
      {
         rejectReason = StringFormat("SEG15M_FILTER_15-%03d", seg.segment15M);
         if(EnableDebugMode)
            Print(StringFormat("⏱️ Time Filter BLOCKED: 15-min segment %d outside range [%d-%d]", 
                  seg.segment15M, Segment15M_Min, Segment15M_Max));
         return false;
      }
   }
   
   // 30-Minute Segment Filter (IN_Segment_30M_OP_01: 30-001 to 30-048)
   if(UseSegment30M)
   {
      if(seg.segment30M < Segment30M_Min || seg.segment30M > Segment30M_Max)
      {
         rejectReason = StringFormat("SEG30M_FILTER_30-%03d", seg.segment30M);
         if(EnableDebugMode)
            Print(StringFormat("⏱️ Time Filter BLOCKED: 30-min segment %d outside range [%d-%d]", 
                  seg.segment30M, Segment30M_Min, Segment30M_Max));
         return false;
      }
   }
   ...
   if(UseSegment01H)
   {
      if(seg.segment01H < Segment01H_Min || seg.segment01H > Segment01H_Max)
      {
         rejectReason = StringFormat("SEG01H_FILTER_1h-%03d", seg.segment01H);
         if(EnableDebugMode)
            Print(StringFormat("⏱️ Time Filter BLOCKED: 1-hour segment %d outside range [%d-%d]", 
                  seg.segment01H, Segment01H_Min, Segment01H_Max));
         return false;
      }
   }
   ...
   if(UseSegment02H)
   {
      if(seg.segment02H < Segment02H_Min || seg.segment02H > Segment02H_Max)
      {
         rejectReason = StringFormat("SEG02H_FILTER_2h-%03d", seg.segment02H);
         if(EnableDebugMode)
            Print(StringFormat("⏱️ Time Filter BLOCKED: 2-hour segment %d outside range [%d-%d]", 
                  seg.segment02H, Segment02H_Min, Segment02H_Max));
         return false;
      }
   }
   ...
   if(UseSegment03H)
   {
      if(seg.segment03H < Segment03H_Min || seg.segment03H > Segment03H_Max)
      {
         rejectReason = StringFormat("SEG03H_FILTER_3h-%03d", seg.segment03H);
         if(EnableDebugMode)
            Print(StringFormat("⏱️ Time Filter BLOCKED: 3-hour segment %d outside range [%d-%d]", 
                  seg.segment03H, Segment03H_Min, Segment03H_Max));
         return false;
      }
   }
   ...
   if(UseSegment04H)
   {
      if(seg.segment04H < Segment04H_Min || seg.segment04H > Segment04H_Max)
      {
         rejectReason = StringFormat("SEG04H_FILTER_4h-%03d", seg.segment04H);
         if(EnableDebugMode)
            Print(StringFormat("⏱️ Time Filter BLOCKED: 4-hour segment %d outside range [%d-%d]", 
                  seg.segment04H, Segment04H_Min, Segment04H_Max));
         return false;
      }
   }
...
int OnInit()
{
   ...
   // Time Segment Filters (CST-based, matches CSV data model)
   bool hasTimeFilters = UseDayFilter || UseSegment15M || UseSegment30M || UseSegment01H || UseSegment02H || UseSegment03H || UseSegment04H;
   if(hasTimeFilters)
   {
      Print("   ⏰ TIME SEGMENT FILTERS (CST = MT5 - 8hrs):");
      
      if(UseDayFilter)
      {
         string allowedDays = "";
         if(AllowSunday) allowedDays += "Sun ";
         if(AllowMonday) allowedDays += "Mon ";
         if(AllowTuesday) allowedDays += "Tue ";
         if(AllowWednesday) allowedDays += "Wed ";
         if(AllowThursday) allowedDays += "Thu ";
         if(AllowFriday) allowedDays += "Fri ";
         if(AllowSaturday) allowedDays += "Sat ";
         Print(StringFormat("      → Day Filter (IN_CST_Day_OP_01): %s", allowedDays));
      }
      
      if(UseSegment15M)
         Print(StringFormat("      → 15M Segment (IN_Segment_15M_OP_01): 15-%03d to 15-%03d", Segment15M_Min, Segment15M_Max));
      
      if(UseSegment30M)
         Print(StringFormat("      → 30M Segment (IN_Segment_30M_OP_01): 30-%03d to 30-%03d", Segment30M_Min, Segment30M_Max));
      
      if(UseSegment01H)
         Print(StringFormat("      → 1H Segment (IN_Segment_01H_OP_01): 1h-%03d to 1h-%03d", Segment01H_Min, Segment01H_Max));
      
      if(UseSegment02H)
         Print(StringFormat("      → 2H Segment (IN_Segment_02H_OP_01): 2h-%03d to 2h-%03d", Segment02H_Min, Segment02H_Max));
      
      if(UseSegment03H)
         Print(StringFormat("      → 3H Segment (IN_Segment_03H_OP_01): 3h-%03d to 3h-%03d", Segment03H_Min, Segment03H_Max));
      
      if(UseSegment04H)
         Print(StringFormat("      → 4H Segment (IN_Segment_04H_OP_01): 4h-%03d to 4h-%03d", Segment04H_Min, Segment04H_Max));
   }
...
void OnNewBar()
{
   ...
   // Time Segment Filters (CST-based, check BEFORE physics filters)
   // Uses CheckTimeSegmentFilters() which handles all segment types
   if(!BypassAllFilters)
   {
      string timeRejectReason = "";
      if(!CheckTimeSegmentFilters(timeRejectReason))
      {
   ...
   if((UseMAForEntry || UsePhysicsEntry) && UsePhysicsFilters && !BypassAllFilters)
   {
      ...
      // Acceleration filter (with MIN floor and MAX ceiling)
      if(UseAccelerationFilter && passFilters)
      {
         ...
         else if(signal > 0 && accel > MaxAccelerationBuy)
         {
            passFilters = false;
            rejectReason = StringFormat("Accel_Spike_%.1f>%.1f", accel, MaxAccelerationBuy);
            if(EnableDebugMode)
               Print(StringFormat("❌ CEILING FAIL: Acceleration %s > %.1f (BUY spike protection)", DoubleToString(accel, 2), MaxAccelerationBuy));
         }
         ...
         else if(signal < 0 && accel < MaxAccelerationSell)
         {
            passFilters = false;
            rejectReason = StringFormat("Accel_Spike_%.1f<%.1f", accel, MaxAccelerationSell);
            if(EnableDebugMode)
               Print(StringFormat("❌ CEILING FAIL: Acceleration %s < %.1f (SELL spike protection)", DoubleToString(accel, 2), MaxAccelerationSell));
         }
      }

      // Speed filter (with MIN floor and MAX ceiling)
      if(UseSpeedFilter && passFilters)
      {
         ...
         else if(signal > 0 && speed > MaxSpeedBuy)
         {
            passFilters = false;
            rejectReason = StringFormat("Speed_Spike_%.1f>%.1f", speed, MaxSpeedBuy);
            if(EnableDebugMode)
               Print(StringFormat("❌ CEILING FAIL: Speed %s > %.1f (BUY spike protection)", DoubleToString(speed, 2), MaxSpeedBuy));
         }
         ...
         else if(signal < 0 && speed < MaxSpeedSell)
         {
            passFilters = false;
            rejectReason = StringFormat("Speed_Spike_%.1f<%.1f", speed, MaxSpeedSell);
            if(EnableDebugMode)
               Print(StringFormat("❌ CEILING FAIL: Speed %s < %.1f (SELL spike protection)", DoubleToString(speed, 2), MaxSpeedSell));
         }
      }
      
      // Momentum filter (with MIN floor and MAX ceiling) - FIXED
      if(UseMomentumFilter && passFilters)
      {
         ...
         else if(signal > 0 && momentum > MaxMomentumBuy)
         {
            passFilters = false;
            rejectReason = StringFormat("Momentum_Spike_%.1f>%.1f", momentum, MaxMomentumBuy);
            if(EnableDebugMode)
               Print(StringFormat("❌ CEILING FAIL: Momentum %s > %.1f (BUY spike protection)", DoubleToString(momentum, 2), MaxMomentumBuy));
         }
         ...
         else if(signal < 0 && momentum < MaxMomentumSell)
         {
            passFilters = false;
            rejectReason = StringFormat("Momentum_Spike_%.1f<%.1f", momentum, MaxMomentumSell);
            if(EnableDebugMode)
               Print(StringFormat("❌ CEILING FAIL: Momentum %s < %.1f (SELL spike protection)", DoubleToString(momentum, 2), MaxMomentumSell));
         }
      }
      ...
      // v4.5: Slope Filters (Directional Momentum Confirmation) with v5.0.0.5 Ceiling Protection
      if(UseSlopeFilters && passFilters)
      {
         // Calculate all slopes
         CalculatePhysicsSlopes();
         
         // Speed Slope Filter (with ceiling)
         if(UseSpeedSlope)
         {
            ...
            else if(signal > 0 && g_lastSpeedSlope > MaxSpeedSlopeBuy)
            {
               passFilters = false;
               rejectReason = StringFormat("SpeedSlope_Spike_%.2f>%.2f", g_lastSpeedSlope, MaxSpeedSlopeBuy);
               if(EnableDebugMode)
                  Print(StringFormat("❌ CEILING FAIL: Speed Slope %s > %s (BUY spike protection)", DoubleToString(g_lastSpeedSlope, 2), DoubleToString(MaxSpeedSlopeBuy, 2)));
            }
            ...
            else if(signal < 0 && g_lastSpeedSlope < MaxSpeedSlopeSell)
            {
               passFilters = false;
               rejectReason = StringFormat("SpeedSlope_Spike_%.2f<%.2f", g_lastSpeedSlope, MaxSpeedSlopeSell);
               if(EnableDebugMode)
                  Print(StringFormat("❌ CEILING FAIL: Speed Slope %s < %s (SELL spike protection)", DoubleToString(g_lastSpeedSlope, 2), DoubleToString(MaxSpeedSlopeSell, 2)));
            }
         }
         
         // Acceleration Slope Filter (with ceiling)
         if(UseAccelerationSlope && passFilters)
         {
            ...
            else if(signal > 0 && g_lastAccelerationSlope > MaxAccelerationSlopeBuy)
            {
               passFilters = false;
               rejectReason = StringFormat("AccelSlope_Spike_%.2f>%.2f", g_lastAccelerationSlope, MaxAccelerationSlopeBuy);
               if(EnableDebugMode)
                  Print(StringFormat("❌ CEILING FAIL: Accel Slope %s > %s (BUY spike protection)", DoubleToString(g_lastAccelerationSlope, 2), DoubleToString(MaxAccelerationSlopeBuy, 2)));
            }
            ...
            else if(signal < 0 && g_lastAccelerationSlope < MaxAccelerationSlopeSell)
            {
               passFilters = false;
               rejectReason = StringFormat("AccelSlope_Spike_%.2f<%.2f", g_lastAccelerationSlope, MaxAccelerationSlopeSell);
               if(EnableDebugMode)
                  Print(StringFormat("❌ CEILING FAIL: Accel Slope %s < %s (SELL spike protection)", DoubleToString(g_lastAccelerationSlope, 2), DoubleToString(MaxAccelerationSlopeSell, 2)));
            }
         }
         
         // Confluence Slope Filter (must be increasing for both BUY and SELL, with ceiling)
         if(UseConfluenceSlope && passFilters)
         {
            ...
            else if(signal > 0 && g_lastConfluenceSlope > MaxConfluenceSlopeBuy)
            {
               passFilters = false;
               rejectReason = StringFormat("ConfluenceSlope_Spike_Buy_%.2f>%.2f", g_lastConfluenceSlope, MaxConfluenceSlopeBuy);
               if(EnableDebugMode)
                  Print(StringFormat("❌ CEILING FAIL: Confluence Slope %s > %s (BUY spike protection)", DoubleToString(g_lastConfluenceSlope, 2), DoubleToString(MaxConfluenceSlopeBuy, 2)));
            }
            ...
            else if(signal < 0 && g_lastConfluenceSlope > MaxConfluenceSlopeSell)
            {
               passFilters = false;
               rejectReason = StringFormat("ConfluenceSlope_Spike_Sell_%.2f>%.2f", g_lastConfluenceSlope, MaxConfluenceSlopeSell);
               if(EnableDebugMode)
                  Print(StringFormat("❌ CEILING FAIL: Confluence Slope %s > %s (SELL spike protection)", DoubleToString(g_lastConfluenceSlope, 2), DoubleToString(MaxConfluenceSlopeSell, 2)));
            }
         }
         
         // Momentum Slope Filter (with ceiling)
         if(UseMomentumSlope && passFilters)
         {
            ...
            else if(signal > 0 && g_lastMomentumSlope > MaxMomentumSlopeBuy)
            {
               passFilters = false;
               rejectReason = StringFormat("MomentumSlope_Spike_%.2f>%.2f", g_lastMomentumSlope, MaxMomentumSlopeBuy);
               if(EnableDebugMode)
                  Print(StringFormat("❌ CEILING FAIL: Momentum Slope %s > %s (BUY spike protection)", DoubleToString(g_lastMomentumSlope, 2), DoubleToString(MaxMomentumSlopeBuy, 2)));
            }
            ...
            else if(signal < 0 && g_lastMomentumSlope < MaxMomentumSlopeSell)
            {
               passFilters = false;
               rejectReason = StringFormat("MomentumSlope_Spike_%.2f<%.2f", g_lastMomentumSlope, MaxMomentumSlopeSell);
               if(EnableDebugMode)
                  Print(StringFormat("❌ CEILING FAIL: Momentum Slope %s < %s (SELL spike protection)", DoubleToString(g_lastMomentumSlope, 2), DoubleToString(MaxMomentumSlopeSell, 2)));
            }
         }
         
         // Jerk Slope Filter (advanced, may be noisy, with ceiling)
         if(UseJerkSlope && passFilters)
         {
            ...
            else if(signal > 0 && g_lastJerkSlope > MaxJerkSlopeBuy)
            {
               passFilters = false;
               rejectReason = StringFormat("JerkSlope_Spike_%.2f>%.2f", g_lastJerkSlope, MaxJerkSlopeBuy);
               if(EnableDebugMode)
                  Print(StringFormat("❌ CEILING FAIL: Jerk Slope %s > %s (BUY spike protection)", DoubleToString(g_lastJerkSlope, 2), DoubleToString(MaxJerkSlopeBuy, 2)));
            }
            ...
            else if(signal < 0 && g_lastJerkSlope < MaxJerkSlopeSell)
            {
               passFilters = false;
               rejectReason = StringFormat("JerkSlope_Spike_%.2f<%.2f", g_lastJerkSlope, MaxJerkSlopeSell);
               if(EnableDebugMode)
                  Print(StringFormat("❌ CEILING FAIL: Jerk Slope %s < %s (SELL spike protection)", DoubleToString(g_lastJerkSlope, 2), DoubleToString(MaxJerkSlopeSell, 2)));
            }
         }
      }
