//+------------------------------------------------------------------+
//|                         TP_Integrated_EA_Crossover_3_1_05M.mq5   |
//|                      TickPhysics Institutional Framework (ITPF)  |
//|           Physics-Optimized EA - Zone/Regime/Time Filtering      |
//+------------------------------------------------------------------+
#property copyright "Copyright 2025, QuanAlpha"
#property link      "https://github.com/quanalpha/tickphysics"
#property version   "3.1"
#property description "v3.1_05M: Zone/Regime/Time filters based on v3.0 baseline analysis"
#property description "Filters: TRANSITION zone (21.8% WR), LOW regime (23.0% WR), Hours 6,7,13,14 (<20% WR)"

// EA Version Info (for CSV tracking)
#define EA_NAME "TP_Integrated_EA"
#define EA_VERSION "3.1_05M"

input int MagicNumber = 300311;                // EA magic number (v3.1 = 300311)
input string TradeComment = "TP_Integrated 3_1_05M";   // Trade comment
...
input int StopLossPips = 0;                    // Stop loss (pips) - 0 = DISABLED
input int TakeProfitPips = 0;                  // Take profit (pips) - 0 = DISABLED
...
// === Signal Filters - v3.1 OPTIMIZATIONS ===
input group "🎯 Physics Filters - v3.1 ZONE/REGIME/TIME"
input bool UsePhysicsFilters = true;            // Enable physics filtering - v3.1 ENABLED
input double MinQuality = 70.0;                // Min physics quality (baseline)
input double MinConfluence = 70.0;             // Min confluence (baseline)
input bool UseZoneFilter = true;                // Filter by trading zone - v3.1 ENABLED
input bool UseRegimeFilter = true;              // Filter by volatility regime - v3.1 ENABLED
input bool UseTimeFilter = true;                // Filter by time of day - v3.1 NEW!
input string AllowedHours = "";                 // Allowed hours (comma-separated, empty = all except blocked)
input string BlockedHours = "6,7,13,14";       // Blocked hours (<20% WR) - v3.1 DATA-DRIVEN
...
{
   Print("═══════════════════════════════════════════════════════");
   Print("🚀 TickPhysics Integrated EA v3.1_05M - Initializing");
   Print("═══════════════════════════════════════════════════════");
   ...
   Print("");
   Print("✅ ALL SYSTEMS READY - v3.1_05M ZONE/REGIME/TIME OPTIMIZATION!");
   Print("═══════════════════════════════════════════════════════");
   Print("📋 Configuration:");
   Print("   EA Version: v3.1_05M (Zone/Regime/Time Optimized)");
   Print("   🎯 OPTIMIZATION: Filter worst zones, regimes, and hours from v3.0 analysis");
   ...
   {
      Print("   → Zone Filter: ", UseZoneFilter ? "ENABLED (avoid TRANSITION 21.8% WR)" : "DISABLED");
      Print("   → Regime Filter: ", UseRegimeFilter ? "ENABLED (avoid LOW 23.0% WR)" : "DISABLED");
      Print("   → Time Filter: ", UseTimeFilter ? "ENABLED (block hours " + BlockedHours + ")" : "DISABLED");
      Print("   → Quality Filter: MinQuality=", MinQuality, " (baseline)");
      Print("   → Confluence Filter: MinConfluence=", MinConfluence, " (baseline)");
   }
   ...
   Print("   CSV Files: v3.1_05M (Trades & Signals)");
   Print("═══════════════════════════════════════════════════════");
...
{
   Print("═══════════════════════════════════════════════════════");
   Print("🛑 TickPhysics Integrated EA v3.1_05M - Shutting Down");
   Print("   Reason: ", reason);
   Print("═══════════════════════════════════════════════════════");
...
void OnNewBar()
{
   ...
   // 4. Apply v3.1 physics filters
   string rejectReason = "";
   bool passFilters = true;
   
   if(UsePhysicsFilters)
   {
      // Quality/Confluence filters (baseline thresholds)
      ...
      // v3.1 Zone filter: AVOID TRANSITION zone (21.8% WR in v3.0 baseline)
      else if(UseZoneFilter && zone == ZONE_TRANSITION)
      {
         rejectReason = "TRANSITION zone (21.8% WR - WORST)";
         passFilters = false;
      }
      // v3.1 Regime filter: AVOID LOW volatility (23.0% WR in v3.0 baseline)
      else if(UseRegimeFilter && regime == REGIME_LOW)
      {
         rejectReason = "LOW volatility regime (23.0% WR - WORST)";
         passFilters = false;
      }
      ...
      {
         if(EnableDebugMode)
            Print("🚫 v3.1 Signal rejected: ", rejectReason, " (Zone=", g_physics.GetZoneName(zone), ", Regime=", g_physics.GetRegimeName(regime), ")");
         return;
      }
      else if(EnableDebugMode)
      {
         Print("✅ v3.1 Signal ACCEPTED: Zone=", g_physics.GetZoneName(zone), ", Regime=", g_physics.GetRegimeName(regime));
      }
   }
   ...
   {
      // No stop loss - use minimal fixed lot size
      lots = SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_MIN);
      
      if(EnableDebugMode)
         Print("📏 Fixed lot size ", lots, " (no SL/TP)");
   }
   else
   {
   ...
   // 11. Execute trade
   if(EnableDebugMode)
      Print("📊 v3.1 Signal confirmed: ", (signal > 0 ? "BUY" : "SELL"), " | Closed opposite: ", closedCount);
   
   Print("🚀 v3.1_05M EXECUTING TRADE NOW...");
                  ...
                  double quality, double confluence, TRADING_ZONE zone, VOLATILITY_REGIME regime)
{
   ...
   Print("✅ v3.1_05M Position opened: #", ticket);
