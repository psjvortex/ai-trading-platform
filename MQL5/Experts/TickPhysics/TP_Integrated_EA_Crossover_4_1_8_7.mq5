//+------------------------------------------------------------------+
//|                       TP_Integrated_EA_Crossover_4_1_8_7.mq5     |
//|                      TickPhysics Institutional Framework (ITPF)  |
//|             v4.187 - SLOPE ANALYSIS (QA APPROVED)                |
//+------------------------------------------------------------------+

#property copyright "Copyright 2025, QuanAlpha"
#property link      "https://github.com/quanalpha/tickphysics"
#property version   "4.187"
#property description "v4.187 EA with Slope Analysis + Universal Elite v3.1 Indicator Support - QA APPROVED"

// EA Version Info (for CSV tracking)
#define EA_NAME "TP_Integrated_EA"
#define EA_VERSION "4.187_SLOPE"

input int MagicNumber = 4004187;                        // EA magic number
input string TradeComment = "TP_Integrated 4_187";      // Trade comment
...
input int         StopLossPips            = 50000;        // Stop loss (pips) - REALISTIC DEFAULT
input int         TakeProfitPips          = 50000;       // Take profit (pips) - 2:1 R:R
...
void LogCompletedTrade(ClosedTrade &trade)
{
   ...
   // v8.0: CRITICAL - Calculate physics decay and derived metrics
   ...
   entrySnapshot.quality = trade.entryQuality;
   entrySnapshot.confluence = trade.entryConfluence;
   entrySnapshot.momentum = trade.entryMomentum;
   entrySnapshot.entropy = trade.entryEntropy;
   entrySnapshot.physicsScore = trade.entryPhysicsScore;
   entrySnapshot.zone = trade.entryZone;
   entrySnapshot.regime = trade.entryRegime;
