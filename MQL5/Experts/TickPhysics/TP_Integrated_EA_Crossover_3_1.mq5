//+------------------------------------------------------------------+
//|                         TP_Integrated_EA_Crossover_3_1.mq5       |
//|                      TickPhysics Institutional Framework (ITPF)  |
//|         DATA-DRIVEN OPTIMIZATION - Zone/Regime/Time Filtering    |
//+------------------------------------------------------------------+
#property copyright "Copyright 2025, QuanAlpha"
#property link      "https://github.com/quanalpha/tickphysics"
#property version   "3.1"
#property description "v3.1: Data-driven optimization from v3.0 baseline analysis"
#property description "Enabled: Zone filter (avoid BEAR), Regime filter (avoid LOW), Time filter (optimal hours)"

// EA Version Info (for CSV tracking)
#define EA_NAME "TP_Integrated_EA"
#define EA_VERSION "3.1"

input int MagicNumber = 300301;                // EA magic number (v3.1)
input string TradeComment = "TP_Integrated 3_1";   // Trade comment
...
// === Signal Filters ===
input group "🎯 Physics Filters - v3.1 DATA-DRIVEN OPTIMIZATION"
input bool UsePhysicsFilters = true;            // Enable physics filtering - v3.1 OPTIMIZED
input double MinQuality = 70.0;                // Min physics quality (not strongly correlated)
input double MinConfluence = 70.0;             // Min confluence (not strongly correlated)
input bool UseZoneFilter = true;                // Filter by trading zone - v3.1 ENABLED (avoid BEAR 19% WR)
input bool UseRegimeFilter = true;              // Filter by volatility regime - v3.1 ENABLED (avoid LOW 21.2% WR)
...
{
   Print("═══════════════════════════════════════════════════════");
   Print("🚀 TickPhysics Integrated EA v3.1 - DATA-DRIVEN OPTIMIZATION");
   Print("═══════════════════════════════════════════════════════");
   Print("📊 Based on v3.0 baseline analysis (454 trades, 28.0% WR)");
   Print("🎯 Optimization: Zone/Regime/Time filters enabled");
   Print("═══════════════════════════════════════════════════════");
   ...
   Print("");
   Print("✅ ALL SYSTEMS READY - v3.1 DATA-DRIVEN OPTIMIZATION!");
   Print("═══════════════════════════════════════════════════════");
   Print("📋 Configuration:");
   Print("   EA Version: v3.1 (Data-Driven from v3.0 Baseline)");
   Print("   🎯 OPTIMIZATION ENABLED: Zone/Regime/Time Filters");
   ...
   {
      Print("   → Zone Filter: ", UseZoneFilter ? "ENABLED (Avoid BEAR - 19.0% WR)" : "DISABLED");
      Print("   → Regime Filter: ", UseRegimeFilter ? "ENABLED (Avoid LOW - 21.2% WR)" : "DISABLED");
   ...
   Print("   SL/TP: ", StopLossPips, "/", TakeProfitPips, " pips (still disabled for v3.1)");
   Print("   Post-Exit Monitor: ", PostExitMonitorBars, " bars");
   Print("   CSV Files: ", "v3.1 (Trades & Signals)");;;
   Print("═══════════════════════════════════════════════════════");
...
void OnNewBar()
{
   ...
   // 5. Apply physics filters (v3.1 OPTIMIZATION ENABLED)
   string rejectReason = "";
