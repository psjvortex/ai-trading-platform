//+------------------------------------------------------------------+
//| TickPhysics_AllSymbols_Indicator_v2_1.mq5                           |
//| Crypto-Optimized Elite Momentum Physics Engine                   |
//| Version 2.0 - Enhanced: Entropy Filter, Learning, Fixes          |
//| CRITICAL FIXES: Works with EA v2.0 (SL/TP bug fixed)             |
//+------------------------------------------------------------------+
...
//input string InpAsset = "BITCOIN";   // Asset (BTCUSD/ETHUSD/BTCUSDT)
                ...
                const long &volume[], const int &spread[])
{
   ...
   for(int i = start; i >= 0; i--)
   {
      ...
      CalculateEntropy(i, total, tick_volume);  // NEW v2.0: Entropy calculation
...
void CalculateConfluence(int i, int total, double highThr)
{
   if(InpShowConfluence && i <= total - InpSpeedPeriod - 2 && i < ArraySize(ConfluenceBuffer))
   {
      ...
      if(i < ArraySize(DistanceROCBuffer) && i < ArraySize(SpeedBuffer) && DistanceROCBuffer[i] * SpeedBuffer[i] > 0) confluence++;
...
//+------------------------------------------------------------------+
//| Calculate Tick Entropy (v2.0 Feature - Market Chaos Measure)    |
//+------------------------------------------------------------------+
void CalculateEntropy(int i, int total, const long &tick_volume[])
{
   ...
   // Calculate entropy using tick volume variance as chaos proxy
   // High entropy = unpredictable/choppy market
   // Low entropy = trending/predictable market
   
   double mean = 0.0;
   int validCount = 0;
   
   // Calculate mean tick volume over window
   ...
   // This represents relative volatility (entropy)
   if(mean > 0)
   {
      double stdDev = MathSqrt(variance);
      double entropy = stdDev / mean;  // Coefficient of variation
      
      // Scale to reasonable range (typically 0.0 to 5.0)
      // Values > 2.5 indicate high chaos/unpredictability
      EntropyBuffer[i] = entropy;
      
      // Store in divergence history for additional analysis
...
void OnDeinit(const int reason)
{
   ...
   Print("TickPhysics Crypto Weekend v1.0 deinitialized. Reason: ", reason);
}
//+------------------------------------------------------------------+
